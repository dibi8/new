...
def format_linkedin(article: Article) -> str:
    """Generate a varied LinkedIn post (professional tone)."""
    hashtags = pick_hashtags(article, count=random.randint(3, 5))
    hook = random.choice(LONG_HOOKS)
...
def format_medium_html(article: Article) -> str:
    """Generate varied Medium article HTML."""
    intro = random.choice(MEDIUM_INTROS)
    hashtags = pick_hashtags(article, count=3)

    return f"""<h1>{article.title}</h1>
<p><em>{intro}</em></p>
<p>{article.description}</p>
<hr>
<p><strong>Read the full guide on dibi8:</strong> <a href="{article.url}">{article.url}</a></p>
<p>{hashtags}</p>"""
